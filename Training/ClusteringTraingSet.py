...
def UpdateEnrichedData(con, m, j):
    
    "Setup the trend type Data"
    enrichedData = {}
    post_date = j[1]
    name = j[6]
    trend_type = j[7]
    ...
    update_enriched_data(con, enrichedData)
    m = m + 1
    if m % 1000 == 0:
        con.commit()
...
def update_enriched_data(conn,enrichedData):
    cur = conn.cursor()
    sql = "update t_enriched_bloomberg_prices set trend_type=? where name=? and post_date=? and type=?"
    ...
    cur.execute(sql,([trendType,name,postDate,ty]))
