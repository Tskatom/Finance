...
def arg_parser():
    ...
    ap.add_argument('-f',dest="news_file",type=str,help="The input path of news file")
...
def insert_enriched_news(conn,enriched_news):
    try:
        ...
        content = json.dumps(enriched_news["content"])
...
def initiate(news_file,conn):
    for line in open(news_file):
        news = json.loads(line)
        news["derivedFrom"] = json.dumps([news["embersParentId"]])
        news["updateTime"] = news["date"]
        insert_enriched_news(conn,news)
    conn.commit()
