...
class GSR_Event:
    ...
    def load_gsr(self):
        ...
        for i in range(1, g_len):
            ...
            if re.search('^04', event_code):
                event = {"eventId": event_id, "eventSubId": event_sub_id, "country": country, "state": state, "city": city, "eventCode": event_code, "population": population, "eventDate": event_date, "earliestDate": earliest_date, "source": source}
    ...
    def commit_db(self):
        ...
        sql = "insert into gsr_event (event_id, event_sub_id,country,state,city,event_code,population,\
                event_date,earlist_date) values (?,?,?,?,?,?,?,?,?)"
        c = self.db_conn.cursor()
        ...
        for event in self.gsr_events:
            ...
            #source = event["source"]
            try:
                c.execute(sql, [event_id, event_sub_id, country, state, city, event_code, population, event_date, earliest_date])
