...
from etool import queue, logs, args
import json
import math
import operator
import hashlib
...
Data flow:
    1> retrieve 3 past day's news from sqlite
        database table: t_daily_enrichednews
    2> retrieve 3 past day's trend type from sqlite
        database table: t_enriched_bloomberg_prices
    3> predict the stock trend type of the day to be predicted
        (This is surrogate data, which is sent to ZMQ and stored
        in sqlite table: t_surrogatedata)
    4> according to the stock predicted trend type, check if
        it will cause a sigma event (If this is a warning,
        it will be sent to ZMQ and be stored in table: t_warningmessage )
...
def check_if_tradingday(domain, predictiveDate, stockIndex):
    "Check if the day weekend"
    weekDay = datetime.strptime(predictiveDate, "%Y-%m-%d").weekday()
    if weekDay == 5 or weekDay == 6:
        log.info("%s For %s is Weekend, Just Skip!"
                 % (predictiveDate, stockIndex))
        return False

    "Check if the day is holiday"
    sql = "select count(*) from s_holiday where "
    sql += " stockIndex = '{}' and date='{}' "
    sql = sql.format(stockIndex, predictiveDate)
    rs = domain.select(sql)
    count = 0
    for r in rs:
        count = int(r['Count'])
    if count == 0:
        ...
        log.info("%s For %s is Holiday, Just Skip!"
                 % (predictiveDate, stockIndex))
        return False


def get_past_cluster_list(domain, predict_date, stock_index, duration=3):
...
def get_term_list(domain, predict_date, stock_index, duration=3):
    ...
    termList, news_derived = query_keywords(domain, start_day, end_day, stock_index)
    return termList, news_derived


# calculate the stock index contribution for the coming day
def compute_stock_index_probability(predict_date, cluster_type, stock_index, cluster_types_history):
    try:
        ...
        for key in cluster_contribution_json[stock_index].keys():
            if key == str(cluster_type):
                ...
                stock_probability = stock_probability + \
                    math.log(float(cluster_contribution_json[stock_index][key][int(cluster_types_history[0]) - 1][2])) + \
                    math.log(float(cluster_contribution_json[stock_index][key][int(cluster_types_history[1]) - 1][1])) + \
                    math.log(float(cluster_contribution_json[stock_index][key][int(cluster_types_history[2]) - 1][0])) + \
                    math.log(float(cluster_json[str(cluster_type)]))
...
def compute_stock_news_probability(predict_date, cluster_type, stock_index, termList):
    try:
                    ...
                    for word, count in termList.iteritems():
                        if word in stermlist:
                            ...
                            term_probability = term_probability + \
                                count * math.log(float(term_contribution_json[stock_index][term_cluster_type][word]))
...
def insert_surrogatedata(surrogateDomain, surrogateData):
    try:
        ...
        rs = surrogateDomain.select(checkSql)
        count = 0
        for r in rs:
            count = int(r['Count'])
        if count == 0:
            ...
            surrogateDomain.put_attributes(surrogateData["embersId"], surrogateData)
    except Exception as e:
...
def insert_warningmessage(warningDomain, warningMessage):
    try:
        ...
        count = 0
        rs = warningDomain.select(checkSql)
        for r in rs:
            count = int(r['Count'])
        if count == 0:
            "Remove the derivedFrom attribute in warningmessage"
            del(warningMessage["derivedFrom"])
            warningDomain.put_attributes(warningMessage["embersId"], warningMessage)
    except Exception as e:
...
def get_predicion_version(warningDomain, rege_date):
...
def re_training(domain, predict_date, stock_list):
...
def get_domain(conn, domain_name):
    t_domain = conn.lookup(domain_name)
    if t_domain is None:
